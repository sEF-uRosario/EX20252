...
class Results(Page):
    ...
    @staticmethod
    def before_next_page(player: Player, timeout_happened):
        # Envió de intentos correctos
        player.participant.vars['num_failed'] = player.num_failed

        # Ejemplo: +10 por acierto, -5 por fallo (mínimo 0)
        payoff = 10 * player.num_correct - 5 * player.num_failed
        if payoff < 0:
            payoff = 0

        # Envió de pago acumulado en esta app
