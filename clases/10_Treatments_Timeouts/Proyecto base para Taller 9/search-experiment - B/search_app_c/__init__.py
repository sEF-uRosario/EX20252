...
def creating_session(subsession: Subsession):
    for p in subsession.get_players():
        pp = p.participant
        p.DECISION_TURNS = pp.decision_turns_c
        p.decisions_remaining = pp.decision_turns_c
        p.symbol = 'X'
...
class Play(Page):
    ...
    def before_next_page(player: Player, timeout_happened):
        ...
        if player.round_number == C.NUM_ROUNDS:
            random_round_part1_c = random.randint(1, C.NUM_ROUNDS)
            pp.random_round_part1_c = random_round_part1_c
            player_in_selected_round = player.in_round(random_round_part1_c)
            pp.payoff_part1_c = player_in_selected_round.payoff
            pp.payoff_part1 = pp.payoff_part1_a + pp.payoff_part1_b + pp.payoff_part1_c
